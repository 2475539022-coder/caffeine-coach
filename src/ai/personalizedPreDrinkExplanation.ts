...
const FALLBACK_EXPLANATION_DATA_LIMITATION =
  "当前有效记录较少，暂时无法形成稳定的个人规律，本次建议主要依据规则计算结果。";
...
export function buildPersonalizedExplanationDataLimitation(input: {
  ...
  if (input.source === "fallback") return FALLBACK_EXPLANATION_DATA_LIMITATION;
...
export function ruleFallbackExplanation(context: CompactEvidenceContext, reason: string): PersonalizedPreDrinkExplanation {
  return {
    ...
    dataLimitation: buildPersonalizedExplanationDataLimitation({ context, source: "fallback" }),
...
export function mockGeneratePersonalizedExplanation(context: CompactEvidenceContext): ModelPersonalizedPreDrinkExplanation {
  ...
  return {
    ...
    explanation:
      decision === "full_cup"
        ? "规则层判断这杯目前可以饮用，历史证据没有提示需要额外收紧。"
        : "规则层建议这次更谨慎，主要是本次睡前残留或今日累计已经接近需要留意的范围。",
...
export function validatePersonalizedExplanation(
  ...
  if (typeof output.historicalObservation !== "string") issues.push({ code: "schema", message: "historicalObservation 缺失或类型错误。" });
